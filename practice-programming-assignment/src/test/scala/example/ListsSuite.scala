...
class ListsSuite extends munit.FunSuite:
  ...
  test("sum of a few different numbers (10pts)") {
    assertEquals(sum(List(1,2,0,3,4)), 10)
  }
  test("sum of no numbers (10pts)") {
    assert(sum(List()) == 0)
  }
  ...
  test("max of a few more numbers (10pts)") {
    assertEquals(max(List(3, 7, 2, 20, 1, 5)),  20)
  }
